function [ a, cb ] = viewSurf( data, surf, title, background)
...
h=0.3125;
w=0.65; % height in the Y-axis

r=max(surf.coord,[],2)-min(surf.coord,[],2);
w1=h/r(2)*r(1)*3/4;
h1=h/r(2)*r(1); % h/r(2)*r(3)

% size of the brain 0-1 (4/5=0.8, 3/4=0.75)
S=0.77;

% [1,1] Left lateral view
a(1)=axes('position',[0.01 0.01 h*S w]);
...
% [1,1] Right LATERAL view
if cut<t
    a(2)=axes('position',[1-0.01-h*3/4 0.01 h*S w]); 
    ...
    a(2)=axes('position',[1-0.01-h*3/4 0.01 h/r(2)*r(1)*S w]);
...
M=0.26;
% [2,2] Left medial view
a(3)=axes('position',[M 0.01 h*S w]); % [2,1]
trisurf(surf.tri(tl,:),surf.coord(1,vl),surf.coord(2,vl),surf.coord(3,vl),...
    double(data(vl)),'EdgeColor','none');
view(90,0); 
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
...
% SUPERIOR VIEW
a(5)=axes('position',[0.3 0.5 w-0.25 h+0.15]);
trisurf(surf.tri,surf.coord(1,:),surf.coord(2,:),surf.coord(3,:),...
    double(data),'EdgeColor','none');
view(0,90); 
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
...
if cut<t
    % [2,1] Right MEDIAL
    a(4)=axes('position',[1-M-h*3/4 0.01 h*S w]);
    trisurf(surf.tri(tr,:)-cuv,surf.coord(1,vr),surf.coord(2,vr),surf.coord(3,vr),...
        double(data(vr)),'EdgeColor','none');
    view(-90,0);
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
lighting phong; material dull; shading interp;
    ...
    % [2,1] Right MEDIAL
    a(4)=axes('position',[1-M-h*3/4 0.01 h/r(2)*r(1)*S w]); % [2,2]
    trisurf(surf.tri,surf.coord(1,:),surf.coord(2,:),surf.coord(3,:),...
        double(data),'EdgeColor','none');
    view(0,0);
    daspect([1 1 1]); axis tight; camlight; axis vis3d off;
...
set(cb,'Position',[0.35 0.1 0.3 0.03]);
...
set(gcf,'PaperPosition',[0.25 2.5 8 6.5]);
